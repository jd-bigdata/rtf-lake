...
/**
 * 获取几条数据
 * java -cp /home/dd_edw_rtf/rtf/RTFWriter-JDQ3.0-1.0-SNAPSHOT.jar com.jd.dw.rtf.writer.Testing
 * .TestConsumer data.bdp.jd.com fae67eb1-91db-11e6-8623-a4dcbef13e1c
 */
    ...
    try {
      while (!Thread.currentThread().isInterrupted()) {
        ...
        for (ConsumerRecord<byte[], byte[]> record : records) {
          ...
          //讲byte[]转为jdwData格式
          JdwData jdata = serializer.fromBytes(record.value());
          //输出string格式


          System.out.println("[offset]:" + record.offset() + ",[ts]:" + record.timestamp() + ", " +
                  "[key]:" + new String(record.key()
          ) + ",[value]: " + jdata.toString());
