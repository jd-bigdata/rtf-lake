    ...
    void writerEventHandler(Event event) {
        ...
        for (Serializable[] row : rows) {
            ...
            if (afterMap != null && afterMap.size() > 0) {
                RtfData realtimedata = new RtfData(event, databaseName, tableName);
...
/*     */
    void updateEventHandler(Event event) {
        ...
        for (Map.Entry<Serializable[], Serializable[]> row : rows) {
            ...
            Map<CharSequence, CharSequence> beforeMap = getMap(beforeRow, databaseName, tableName);
            Map<CharSequence, CharSequence> afterMap = getMap(afterRow, databaseName, tableName);
    ...
    void deleteEventHandler(Event event) {
        ...
        for (Serializable[] row : rows) {
            ...
            Map<CharSequence, CharSequence> beforeMap = getMap(row, databaseName, tableName);
